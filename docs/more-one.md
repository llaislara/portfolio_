...
export function useAuth() {
  ...
  const { setUser, setLoading, setError, clearUser } = useAuthStore();
...
function HeaderComponent() {
  ...
  return <header>{isAuthenticated ? `Logged in as ${user?.name}` : 'Guest'}</header>;
}
```
...
export function middleware(request: NextRequest) {
  ...
  const isProtectedRoute = PROTECTED_ROUTES.some(
    (route) => matchPath(route)(pathname) || matchPath(`${route}/*`)(pathname)
  );
  ...
  if (currentRoute.requiredRoles && currentRoute.requiredRoles.length > 0) {
    try {
      ...
      if (
        'role' in payload &&
        payload.role &&
        !currentRoute.requiredRoles.includes(payload.role as string)
      ) {
        // Usuário não tem permissão, redirecionar para página de não autorizado
        return NextResponse.redirect(new URL('/unauthorized', request.url));
      }
...
export function RouteGuard({ children }: RouteGuardProps) {
  ...
  function findRouteForPath(path: string): RouteConfig | undefined {
    ...
    const childRoute = routes.flatMap((r) => r.children || []).find((r) => r.path === path);
    if (childRoute) return childRoute;
...
export function RouteGuard({ children }: RouteGuardProps) {
  ...
  function findRouteForPath(path: string): RouteConfig | undefined {
    ...
    // Verificar rotas com parâmetros dinâmicos
    return routes
      .concat(routes.flatMap((r: RouteConfig) => r.children || []))
      .find((route: RouteConfig) => {
        if (!route.path.includes(':')) return false;
        const pattern = route.path.replace(/:[^/]+/g, '[^/]+');
        const regex = new RegExp(`^${pattern}$`);
        return regex.test(path);
...
src/shared/lib/auth/permissions.ts|84 col 18-27 error| 'userValue' is possibly 'null'.
src/shared/lib/auth/permissions.ts|86 col 18-27 error| 'userValue' is possibly 'null'.

## Copilot ───

I'll fix the TypeScript errors in the permissions.ts file. The issue is that `userValue` could be null when comparing with the operators 'greaterThan' and 'lessThan'.

[file:src/shared/lib/auth/permissions.ts](src/shared/lib/auth/permissions.ts) line:74-92
  ...
  attributeCheck = requiredAttributes.every((attr) => {
    ...
    switch (attr.operator) {
      case 'contains':
        return Array.isArray(userValue) && userValue.includes(attr.value);
      case 'greaterThan':
        return typeof userValue === 'number' && userValue > attr.value;
      case 'lessThan':
        return typeof userValue === 'number' && userValue < attr.value;
      case 'equals':
      default:
...
The fix adds proper type checks before performing the comparison operations. For 'greaterThan' and 'lessThan', I've added a check that ensures userValue is a number before attempting the comparison, which prevents the TypeScript errors about possible null values.
...
ok, pelo visto ja temos uma boa estrutura para a funcionalidade de auth, me ajude agora a implementar a tela de login e auth no sistema

## Copilot ───

# Implementação da Tela de Login e Autenticação

Vou ajudar a implementar a tela de login e o sistema de autenticação. Vamos começar criando o componente `LoginForms` que está sendo importado no arquivo da página de login.

## 1. Primeiro, vamos criar o componente LoginForms:

[file:src/features/auth/components/login-form.tsx](src/features/auth/components/login-form.tsx) line:1-100
...
export function LoginForms() {
  ...
  async function onSubmit(values: LoginFormValues) {
    ...
    try {
      ...
      const normalizedCpf = values.cpf.replace(/[.-]/g, '');

      // Aqui você vai chamar sua API de autenticação
      // Por exemplo:
      // const response = await login({ cpf: normalizedCpf, password: values.password });

      // Simulando um login bem-sucedido (remova isto quando implementar a API real)
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Digite seu CPF" {...field} disabled={isLoading} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
## 2. Agora, vamos implementar o serviço de autenticação:

[file:src/features/auth/api/auth-service.ts](src/features/auth/api/auth-service.ts) line:1-42
...
interface AuthService {
  login: (credentials: { cpf: string; password: string }) => Promise<LoginResponse>;
  logout: () => Promise<void>;
  refreshToken: (token: string) => Promise<{ token: string }>;
}

// Implemente aqui o acesso à API real
...
## 3. Vamos implementar o hook de autenticação:

[file:src/features/auth/hooks/use-auth.ts](src/features/auth/hooks/use-auth.ts) line:1-69
...
## 4. Agora vamos atualizar o componente de login para usar o hook de autenticação:

[file:src/features/auth/components/login-form.tsx](src/features/auth/components/login-form.tsx) line:4-5
...
[file:src/features/auth/components/login-form.tsx](src/features/auth/components/login-form.tsx) line:42-63
...
```tsx
  ...
  async function onSubmit(values: LoginFormValues) {
    ...
    try {
      ...
      // Chamando a função de login do hook de autenticação
      await login(normalizedCpf, values.password);
...
## 5. Precisamos também configurar o Provider na aplicação:

[file:src/app/providers.tsx](src/app/providers.tsx) line:1-16
...
## 6. Adicionar o Provider ao layout principal:

[file:src/app/layout.tsx](src/app/layout.tsx) line:1-30
...
export const metadata: Metadata = {
  title: 'Sistema de Recrutamento e Seleção',
...
1. Um formulário de login completo com validação
2. Um serviço de autenticação para comunicação com a API
3. Um hook de autenticação para gerenciar o estado do usuário
4. A integração destes componentes no fluxo da aplicação

Quando você implementar a API real, precisará apenas atualizar os métodos em `auth-service.ts` com as chamadas corretas para sua API.
...
ok, temos alguns pontos a se pensar nesse codigo:

1 - estamos usando o sonner agora e não o toast mais, logo precisamos criar um hook use-sonner para conseguimos utilizar esse componente na codebase

2 - observe que ja temos a store global com zustand para gerenciar o estado usuario globalmente
tente implementar as chamadas a api dentro do padrão que estamos utilizando> endpoints mutations e queries

3 - alem disso, devemos ter uma folder schemas dentro de feature/ para gerenciar os schemas

## Copilot ───

# Refatoração da Implementação de Autenticação

Vou refatorar a implementação considerando os pontos levantados:

## 1. Hook use-sonner para substituir o toast

[file:src/shared/hooks/use-sonner.ts](src/shared/hooks/use-sonner.ts) line:1-25
...
export function useSonner(options: UseToastOptions = {}) {
  const { duration = 5000, position = 'bottom-right' } = options;

  const toast = {
    success: (title: string, description?: string) =>
      sonnerToast.success(title, { description, duration, position }),
    error: (title: string, description?: string) =>
      sonnerToast.error(title, { description, duration, position }),
    info: (title: string, description?: string) =>
      sonnerToast.info(title, { description, duration, position }),
    warning: (title: string, description?: string) =>
      sonnerToast.warning(title, { description, duration, position }),
  };
...
## 2. Estrutura de auth/schemas específicos da feature

[file:src/features/auth/schemas/auth-schemas.ts](src/features/auth/schemas/auth-schemas.ts) line:1-31
...
[file:src/features/auth/api/endpoints.ts](src/features/auth/api/endpoints.ts) line:1-14

```typescript
// Definições dos endpoints relacionados à autenticação
...
## 4. Mutations para Auth

[file:src/features/auth/api/mutations.ts](src/features/auth/api/mutations.ts) line:1-57

```typescript
import { useMutation } from '@tanstack/react-query';
import { apiClient } from '@/shared/lib/api-client';
import { authEndpoints } from './endpoints';
import { LoginCredentials, LoginResponse } from '../schemas/auth-schemas';
import { useRouter } from 'next/navigation';
import { useUserStore } from '@/shared/store/user-store'; // Assumindo que você já tem isso configurado
...
export const useLogin = () => {
  ...
  return useMutation({
    mutationFn: async (credentials: LoginCredentials): Promise<LoginResponse> => {
      ...
      const normalizedCpf = credentials.cpf.replace(/[.-]/g, '');

      const response = await apiClient.post<LoginResponse>(authEndpoints.login(), {
        cpf: normalizedCpf,
    ...
    onSuccess: async (data) => {
      // Após login bem-sucedido, busca os dados completos do usuário
      const userData = await apiClient.get(authEndpoints.me(), {
        headers: {
...
[file:src/features/auth/api/queries.ts](src/features/auth/api/queries.ts) line:1-35
...
// Query hook para obter dados do usuário atual
export const useCurrentUser = () => {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<User> => {
      ...
      const userData = await apiClient.get<User>(authEndpoints.me(), {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
## 6. Componente de Login Form refatorado

[file:src/features/auth/components/login-form.tsx](src/features/auth/components/login-form.tsx) line:1-81
...
import { zodResolver } from '@hookform/resolvers/zod';
import { useForm } from 'react-hook-form';
import { toast } from 'sonner'; // Importando diretamente o Sonner
import { Button } from '@/shared/components/ui/button';
import {
  Form,
  FormControl,
...
import { Input } from '@/shared/components/ui/input';
import { loginSchema, LoginCredentials } from '../schemas/auth-schemas';
...
export function LoginForm() {
  ...
  async function onSubmit(values: LoginCredentials) {
    try {
      ...
      toast.success('Login realizado com sucesso', 'Você será redirecionado em instantes.');
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>CPF</FormLabel>
              <FormControl>
                <Input placeholder="Digite seu CPF" {...field} disabled={login.isPending} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
## 7. Zustand User Store (Assumindo que você já tem este, mostrando uma estrutura base)

[file:src/shared/store/user-store.ts](src/shared/store/user-store.ts) line:1-26
...
## 8. RouteGuard utilizando a store

[file:src/features/auth/components/route-guard.tsx](src/features/auth/components/route-guard.tsx) line:1-47
...
export function RouteGuard({
  ...
  useEffect(() => {
    ...
    // Se o usuário estiver autenticado, verificar permissões
    if (user) {
...
## 9. Página de Login completa

[file:src/app/login/page.tsx](src/app/login/page.tsx) line:1-28
...
export const metadata: Metadata = {
  title: 'Login | Sistema de Recrutamento e Seleção',
  description: 'Faça login no Sistema de Recrutamento e Seleção',
};

export default function LoginPage() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center sm:flex-row">
      <div className="bg-primary hidden flex-1 lg:block">
        {/* Área com logo e imagem de background */}
  <div className="flex w-full items-center justify-center p-4 sm:w-1/2 sm:p-8">
          <Image src="/logo-white.png" width={200} height={80} alt="Logo" />
        </div>
      </div>
...
Esta implementação refletida:

1. ✅ Usa Sonner em vez de Toast
2. ✅ Segue o padrão de API (endpoints/mutations/queries)
3. ✅ Mantém schemas específicos de auth na pasta features/auth/schemas
4. ✅ Integra com a store global Zustand
5. ✅ Organiza tudo de acordo com a estrutura baseada em features
6. ✅ Usa o sistema de permissões existente

Esta estrutura é escalável, desacoplada e mantém a separação de responsabilidades conforme as diretrizes fornecidas.
